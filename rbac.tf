resource "null_resource" "db_setup" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      psql -h ${azurerm_postgresql_flexible_server.flexible_server.0.fqdn} -p 5432 -U ${var.entra_admin_user} -d postgres -v 'ON_ERROR_STOP=1' -f ${path.module}/roles/$unique_sql_file_name}.sql
    EOT
    environment = {
