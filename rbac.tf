resource "null_resource" "db_setup" {
  count = length(values(local.group_list))

  triggers = {
    content = templatefile(
      "${path.module}/roles/${values(local.group_list)[count.index].group_name}.sql",
      { groups = [for group in values(local.group_list)[count.index].groups : lower(group)] }
    )
  }
  ...
  provisioner "local-exec" {
    command = <<EOT
      unique_sql_file_name="final_${values(local.group_list)[count.index].group_name}_${local.group_project}.sql"
